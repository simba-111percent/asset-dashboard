...
function parseSwSheetData(sheetData) {
  // 개인 알림 대상에서 제외할 구분 (전사 공통/계약 단위로 총무가 관리, 2026-07-22 확정)
  const PERSONAL_ALERT_EXCLUDED_CATS = ['공통 SaaS', '영구라이선스', 'JetBrains', 'Adobe', 'Unity', 'MS'];
  ...
  for (let i = MASTER_CONFIG.headerRow + 1; i < rows.length; i++) {
    ...
    } else if (PERSONAL_ALERT_EXCLUDED_CATS.includes(cat)) {
      alertType = 'admin'; // 전사 공통/계약 단위 항목은 개인 알림 대상 아님 (총무가 관리)
    } else if (user) {
